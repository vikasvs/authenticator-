...
from .forms import (EmployeeSignUpForm, EmployerSignUpForm, EmployeeDataForm,
                    EmployerDataForm, RecommendationForm, NoModelPersonRecommendationForm)
import logging
import os
    ...
    if request.user.is_employee:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and form.cleaned_data.get('name_of_employee') in [e.your_name for e in employee_list]:
                ...
                global_curr_employee = form.cleaned_data.get(
                    'name_of_employee')
                return redirect('rec-redirect')
            else:
                form = NoModelPersonRecommendationForm(request.POST)
                messages.error(
                    request, 'you are not authorized to recommend this person')
                return redirect('profile-complete')
        else:
...
def RecommendationRedirect(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            employeeUser = Employee.objects.get(
                user=User.objects.get(username=global_curr_employee))
            ...
            print(' the form is {}',
                  form.cleaned_data['offer_letter_validity'])
...
def populateEmployee(request, form, current_user, username):
    ...
    form.cleaned_data['offer_letter'].name = username + \
        form.cleaned_data['offer_letter'].name
